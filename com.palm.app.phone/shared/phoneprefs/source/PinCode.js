...
enyo.kind({
	...
	components: [
		 {kind: "VFlexBox", pack: "center", align: "center", width: "100%", height: "100%",  components: [
			 {kind: "VFlexBox", className: "unlock-pin-screen", width: "400px", height: "500px",  components: [
				{name: "label", components: [
					{name: "pin", content: $L("Device Locked"), style: "text-align: center; color: white; width: 100%; font-size: 26px;"},
					{name: "oldPin", content: $L("Enter old PIN"), style: "font-size: 18px; text-align: center; color: white; width: 100%;"}
				]},	
				{name:"pinlabel", style: "line-height: 18px", content: $L(" "), height: "18px", style: "font-size: 18px; weight: bold; text-align: center; color: #969696; width: 100%; margin-bottom: 10px;"},
				{kind:"PinDialpad", flex: 1, onNumberAdded: "handleButtonClick", onHandleDelete: "HandleBackspace", onHandleClear: "HandleClear"},	
				{kind:"HFlexBox", style: "margin: 10px;", defaultKind: "Button", components: [
					{flex: 1, kind: "Button", height: "50px", layoutKind: "VFlexLayout", pack: "center", name: "btnEmergencyOrCancel", caption: $L("Emergency Call"), onclick: "EmergencyCall", className: "enyo-button-dark"},
					{flex: 1, kind: "Button", height: "50px", layoutKind: "VFlexLayout", pack: "center", caption: $L("Done"), onclick: "DonePinCode", className: "enyo-button-affirmative"}
				]},
			]},
		]},
	...
	create: function() {
		...
		if (enyo.application.isTablet){
			this.$.btnEmergencyOrCancel.setCaption($L("Cancel")); 
		}
		this.initData();
	},	
			...
			switch (this.pinAction) {
				...
				case PinAction.PUK_Enter:
					if (PinStatus.Pukrequired) {
						this.setHeaderText($L("SIM Card PUK Locked"));
						this.$.oldPin.setContent($L("Enter PUK to reset PIN"));
						this.showOldPin();
					}
					break;
				...
				case PinAction.Pin2_ChangeConfirm:
					if (PinStatus.Puk2required) {
						this.setHeaderText($L("Fixed dialing is enabled"));
						this.$.oldPin.setContent($L("Enter PUK2 to reset PIN2"));
						this.showOldPin();
				...
				default:
					enyo.error("PinAction "+this.pinAction+ " is not handled in button pressing");  
					this.setDefault();
					break; 					
			}
			this.pinError = false;
	...
	EmergencyCall: function(){
		if (enyo.application.isTablet) { //this is cancel
			this.doneClick(); 
		}
		else {
	...
	setDefault: function(pin2) {
		this.$.pin.setContent($L({key:"notEASPIN", value: "Enter PIN"})); 
		if (pin2 == true) {
			this.$.pin.setContent($L("Enter PIN2"));
		}
		this.hideOldPin(); 
		this.clearData();
	},	
	...
	setUI: function(){
		...
		switch (this.pinAction){
			case PinAction.PinCode_Change:
				this.$.oldPin.setContent($L("Enter old PIN"));
				this.showOldPin(); 
			break; 

			case PinAction.PinCode_ChangeNew:
			case PinAction.PUK_NewPin:
				this.setHeaderText($L("Enter new PIN")); 
				this.$.oldPin.setContent($L("Enter 4-8 numbers for new PIN")); 
				this.showOldPin(); 
			break; 

			case PinAction.PinCode_ChangeConfirm: 
			case PinAction.PUK_ConfirmPin:
				this.setHeaderText($L("Confirm new PIN"));
				this.$.oldPin.setContent($L("Enter 4-8 numbers for new PIN"));
				this.showOldPin(); 
			break; 
			...
			case PinAction.Pin2_Disable:				
				this.setHeaderText($L("Enter PIN2"));
			break; 			
			 
			case PinAction.Fdn_Verify:
			case PinAction.Fdn_Disable:
				this.setHeaderText($L("Fixed dialing is enabled"));
				this.$.oldPin.setContent($L("Enter PIN2"));
				this.showOldPin();
			break; 

			case PinAction.Pin2_Change:
				this.setHeaderText($L("Enter PIN2")); 
				this.$.oldPin.setContent($L("Enter old PIN2"));
				this.showOldPin();
			break; 

			case PinAction.Pin2_ChangeNew: 
			case PinAction.PUK2_NewPin:
				this.setHeaderText($L("Enter New PIN2")); 
				this.$.oldPin.setContent($L("Enter 4-8 numbers for new PIN2"));
				this.showOldPin();
			break; 

			case PinAction.Pin2_ChangeConfirm:
			case PinAction.PUK2_ConfirmPin:
				this.setHeaderText($L("Confirm new PIN2"));
				this.$.oldPin.setContent($L("Enter 4-8 numbers for new PIN2"));
				this.showOldPin();
			break; 

			case PinAction.PUK_Enter:
				this.setHeaderText($L("Enter PUK"));
				this.$.oldPin.setContent($L("Enter PUK to reset PIN")); 
				this.showOldPin();
			break; 

			case PinAction.PUK2_Enter:
				this.setHeaderText($L("Enter PUK2")); 
				this.$.oldPin.setContent($L("Enter PUK2 to reset PIN2")); 
				this.showOldPin();
			break; 

			case PinAction.SimLocked:
				this.setHeaderText($L("Contact carrier for new SIM")); 
				this.$.oldPin.setContent("");
			break;
			
			case PinAction.PinCode_Verify:
				this.setHeaderText($L("SIM Card Locked")); 
				this.$.oldPin.setContent($L({key: "notEASPIN", value: "Enter PIN"})); 
				this.showOldPin();						
			break; 
			
			case PinAction.deviceLockUnlock:
				if (this.unblock == true) {
					this.setHeaderText($L("Enter Unblock Code"));
					this.$.oldPin.setContent($L("Contact your carrier for the unblock code")); 
				}
				else {
					this.setHeaderText($L("Enter Network Unlock Code"));
					this.$.oldPin.setContent($L("Contact your carrier for the unlock code")); 
				}
				this.showOldPin();
			break; 			
	...
	DonePinCode: function() {
		...
		switch (this.pinAction) {
			...
			case PinAction.SimLocked:
			case PinAction.Exit:
				enyo.log("sim is locked");
				this.doneClick();
			break;

			default:
				//todo: any other conditions??
				enyo.error("NOT HANDLED: " + this.pinAction);
			break; 
		}
	},
	...
	formatRetriesMsg: function(inRetriesNum) {
		return enyo.application.Utils.formatChoice($L("1#1 retry remaining|##{tries} retries remaining"), inRetriesNum, {tries: inRetriesNum});
	},
...
enyo.log(enyo.json.stringify(response));				 
		if (response.returnValue){
			if (this.pinAction == PinAction.PinCode_Lock){
				...
				enyo.windows.addBannerMessage($L("PIN Disabled"), "{}");
			}else{
				enyo.log("unknown action: "+this.pinAction);
			}
			this.doneClick();
		}else{
			...
			switch (response.errorCode){
				case 2:					
					this.setHeaderText($L("PIN incorrect"));
					msg = enyo.application.Messages.enablePinError[response.errorCode].toString();//$L("Unable to change PIN status: bad format."); 
				...
				case 3:
					this.setHeaderText($L("PIN incorrect"));
					this.$.oldPin.setContent($L("Contact carrier for PUK code")); 					
					this.$.oldPin.show();
				...
				default:
					this.setHeaderText($L("PIN incorrect"));
					msg = enyo.application.Messages.enablePinError[0].toString();//$L("Unable to change PIN status.");
					this.$.oldPin.setContent(msg); 
					this.$.oldPin.show(); 					 
					break; 
			}
...
enyo.log(enyo.json.stringify(response));
		if (response.returnValue){
			enyo.windows.addBannerMessage($L("PIN Changed"), "{}");
			this.doneClick();
		}else{
			enyo.error(enyo.json.stringify(response));
			this.pinError = true;
			...
			switch (response.errorCode){
				case 2:
					this.$.pin.setContent($L("PIN incorrect")); 
					if (response.extended && response.extended.attemptsRemaining !== undefined) {
						msg = this.formatRetriesMsg(response.extended.attemptsRemaining);
					}
					else {
				...
				case 6: //Pins don't match
					this.$.pin.setContent($L("Enter new PIN")); 
					msg = $L("PIN doesn't match");
					this.$.oldPin.setContent(msg); 
					this.$.oldPin.show(); 
					this.pinAction = PinAction.PinCode_ChangeNew; 
					break; 
				...
				default:
					this.setHeaderText($L("PIN incorrect"));
					msg = enyo.application.Messages.pinChangeError[0].toString();//$L("Unable to change PIN status.");
					this.$.oldPin.setContent(msg); 
					this.$.oldPin.show(); 
					break;
			}
...
enyo.log(enyo.json.stringify(response));			
		if (response.returnValue) {
			enyo.windows.addBannerMessage($L("PIN2 Changed"), "{}");
			this.doneClick();
		}else{
			enyo.error(enyo.json.stringify(response));
			this.pinError = true;
			...
			switch (response.errorCode){
				case 2:
					this.$.pin.setContent($L("PIN2 incorrect")); 
					if (response.extended && response.extended.attemptsRemaining !== undefined){
						msg = this.formatRetriesMsg(response.extended.attemptsRemaining);
					} else {
						msg = enyo.application.Messages.pinChangeError[response.errorCode].toString();
				...
				case 3:					
					this.setHeaderText($L("PIN incorrect"));
					this.$.oldPin.setContent($L("Contact carrier for PUK2 code")); 
					this.$.oldPin.show();					
				...
				case 5:
					msg = $L("Unable to change PIN2: enable PIN2 first.");
					this.$.oldPin.setContent(msg); 
					this.pinAction = PinAction.Exit; 
					break; 

				case 6: //Pins don't match					
					this.$.pin.setContent($L("Enter new PIN2")); 
					msg = $L("PIN doesn't match");
					this.$.oldPin.setContent(msg); 
					this.$.oldPin.show(); 
					this.pinAction = PinAction.Pin2_changeNew; 
				...
				default:
					this.$.pin.setContent($L("PIN2 incorrect"));
					this.$.oldPin.setContent($L("Unable to Change PIN2")); 
					this.pinAction = PinAction.Pin2_Change;
					break; 
...
enyo.log(enyo.json.stringify(response)); //temp enable the log fro CFISH-797
		if (response.returnValue){
			if (this.pinAction == PinAction.Pin2_Enable){
				...
				enyo.error("unknown action: "+this.pinAction);
			}
			this.doneClick();
		}else{
			enyo.error(enyo.json.stringify(response));
			...
			var msg = ""; 
			if (this.pinAction == PinAction.Pin2_Enable){
				enyo.error("fdnenable error "+response.errorCode);			
			}else if (this.pinAction == PinAction.Pin2_Disable || this.pinAction == PinAction.Fdn_Disable){
				enyo.error("fdnDisable error "+response.errorCode);
			}else{
				enyo.error("unknown action: "+this.pinAction + " errorcode "+response.errorCode);
			}
			switch (response.errorCode){
				case 2:
					this.$.pin.setContent($L("PIN incorrect"));
					msg = enyo.application.Messages.fdnEnableError[response.errorCode].toString();//$L("Unable to change PIN status: PUK locked."); 
				...
				default:
					...
					if (PinStatus.Puk2required == true) {
						...
						this.setHeaderText($L("PIN incorrect"));
						msg = enyo.application.Messages.fdnEnableError[0].toString();//$L("Unable to change PIN status.");
						this.$.oldPin.setContent(msg);
						this.$.oldPin.show();
					} 
					break; 
...
enyo.log(enyo.json.stringify(response));
		if (response.returnValue){
			...
			switch (response.errorCode){
				case 2:
					this.$.pin.setContent($L("PIN2 incorrect")); 
					msg = enyo.application.Messages.pin2VerifyError[response.errorCode].toString();//$L("Unable to change PIN status."); 		
...
enyo.log(enyo.json.stringify(response));		
		if (response.returnValue){
			...
			switch (response.errorCode){
				case 2:
					this.$.pin.setContent($L("PUK incorrect"));
...
enyo.log(enyo.json.stringify(response));				
		if (response.returnValue){
			...
			switch (response.errorCode){
				case 2:
					this.$.pin.setContent($L("PUK2 incorrect"));
					msg = $L("Unable to unlock PUK2: bad or incorrect PUK2.");  
					this.$.oldPin.setContent(msg); 
					this.$.oldPin.show(); 	
					this.pinError = true;			
					PinStatus.Puk2required = true; 
...
enyo.log(enyo.json.stringify(response));
		if (response.returnValue){
			...
			switch (response.errorCode){
				case 2:
					this.$.pin.setContent($L("PIN incorrect")); 
					msg = $L("Unable to verify PIN: bad format."); 		
	...
	DeviceLockUnlockResponse: function(inSender, response) {
		...
		if (response.returnValue){
			...
			switch (response.errorCode){
				case 1:
					enyo.log("power is off"); 
					break; 

				default:
					this.$.pin.setContent($L("Please try again.")); 
					break; 
			}
			this.clearData();
		}
...
enyo.log("nextView "+this.nextView);	
		...
		if (this.nextState) { //we were launched externally by other app or from pin status change
			if (this.nextState === 'preferences_card' && this.nextView) {
				this.doRefreshCard();
				params = {
