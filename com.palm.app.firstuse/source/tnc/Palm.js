label_dialog_palmTnc = rb.$L("HP Terms & Conditions");
label_popup_palm_header = rb.$L("Do you accept?");
label_popup_palm_body = rb.$L("In order to use this device, you must accept the Terms. If you choose not to accept the Terms, you may return this device within the return period of the HP authorized distributor where you purchased this device for a refund subject to the applicable return policy.");

label_button_accept = rb.$L("Accept");
label_button_decline = rb.$L("Decline");
label_button_continue = rb.$L("Continue");

label_popup_palm_error_header = rb.$L("Server Error");
label_popup_palm_error_body = rb.$L("Please visit http://hpwebos.com/support to help resolve this problem");
...
enyo.kind({
	...
	components: [
		...
		{name:"PalmTCs", kind: enyo.VFlexBox,	className: 'section', height:"100%", components: [
			...
			{kind: enyo.Control, layoutKind: "HFlexLayout", align: 'start', pack: 'justify', components: [
				...
				{kind: enyo.Button, className: "enyo-button-affirmative", onclick: "continueButton", caption: label_button_continue}
			]}
		]},
    ...
    getURL: function(){
		...
		if (this.termsAreSet) {
	    	...
	    	console.info("Getting server URL...");
	...
	getTerms: function(inSender, inResponse){
     	var url = "https://" + inResponse.locationHost + "/palmcsext/services/deviceJ/";
