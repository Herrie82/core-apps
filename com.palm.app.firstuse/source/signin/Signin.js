//Header labels
label_header1_signin = rb.$L("HP webOS Account");
label_header2_signin = rb.$L("An HP webOS Account is key to access critical HP services.");
label_header3_signin = rb.$L("If you already have a webOS account (previously Palm Profile), you can sign in now.");
...
label_popup_body_palmProfile = rb.$L("An HP webOS account (previously Palm Profile) is required to activate your device. In addition, your webOS account also gives you:<ul><li>A free account for storing data in Contacts and Calendar</li><li>Backup and restore of personal data and applications downloaded from App Catalog</li><li>Device software updates</li></ul></p>");

label_popup_header_forgotEmail = rb.$L("Forgot Email?");
label_popup_body1_forgotEmail = rb.$L("Visit http://hpwebos.com/support for assistance.");
...
label_popup_body1_forgotPassword = rb.$L("Please enter the email address you used to create your webOS account (previously Palm Profile).");
...
label_popup_body2_recoverPwdSentEmailPopup = rb.$L("If you're unable to reset your password, visit http://hpwebos.com/support for more help.");
...
label_popup_body1_embargoedEmailPopup = rb.$L("The United States Government prohibits HP from allowing you to use this email address to set up a webOS account.");

label_popup_header_embargoedEmailHelpPopup = rb.$L("Invalid Email Address");
label_popup_body1_embargoedEmailHelpPopup = rb.$L("The United States Government restricts exports to certain countries, including the country where your email address domain is issued. You can visit www.palm.com/us/company/legal for additional information.");
...
label_error_unableToSignInText = rb.$L("We were unable to sign you in. Please try again in a few minutes or contact HP for help resolving this problem. Visit http://hpwebos.com/support for more information.");
label_error_unableToCreateAccountText = rb.$L("We are unable to create an account for you. Please try again in a few minutes or contact HP for help resolving this problem. Visit http://hpwebos.com/support for more information.");
label_error_unableToCreateAccountTextNetworkingIssue = rb.$L("We are unable to create a new account right now. Please try again in a few minutes or contact HP for help resolving this problem. Visit http://hpwebos.com/support for more information.");
...
label_error_passwordLengthText = rb.$L("Password must be 6-20 characters.");
...
enyo.kind({
					...
					{name: "introView", kind:"Scroller", components: [
						{kind:"Control", layoutKind: "VFlexLayout", className: 'login', align: 'center', flex:1, components:[
							{kind: "Control", layoutKind: "VFlexLayout", align:'center', className:"contentDefaults", pack: 'justify', components: [
								...
								{content: rb.$L("If you've ever owned an HP or Palm webOS phone or TouchPad, tap Sign In. Otherwise, create a new account."), className: "subtitle"},
					...
					{name: "signInView", kind:"Scroller", components: [
						{kind:"Control", layoutKind: "VFlexLayout", className: 'login', align: 'center', flex:1, components:[
							{kind: "Control", layoutKind: "VFlexLayout", align:'center', className:"contentDefaults2", pack: 'justify', components: [
								...
								{content: rb.$L("Sign in to your HP webOS Account or Palm Profile to copy your applications, accounts and data to your TouchPad."), className: "subtitle"},
					...
					{name: "createView", kind:"Scroller", components: [
						{kind:"Control", layoutKind: "VFlexLayout", className: 'login', align: 'center', flex:1, components:[
							{kind: "Control", layoutKind: "VFlexLayout", align:'center', className:"contentDefaults2", style:"550px", pack: 'justify', components: [
								...
								{content: rb.$L("You need an HP webOS account to use this device. Your data, accounts, application download and purchase history will be securely backed up."), className: "subtitle"},
    	...
    	if(this.$.signInEmail.getValue() != ""){
    		if (!this.verifyEmail(this.$.signInEmail.getValue())) {
				...
				if(!this.verifyEmailLength(this.$.signInEmail.getValue())){
	    			...
	    			if (this.$.signInPassword.getValue().length < 6 || this.$.signInPassword.getValue().length > 20){
						this.$.signInButton.setDisabled(true);
					} else {
	...
	signInPasswordOnChange: function(inSender, inResponse){
		...
		if(!passEmpty) {
			...
			if (l < 6 || l > 20) {
				...
				this.$.signInPasswordLengthText.setShowing(false);
				if (this.verifyEmail(this.$.signInEmail.getValue()) && this.verifyEmailLength(this.$.signInEmail.getValue()) ) {
					if (this.$.signInPassword.getValue().length < 6 || this.$.signInPassword.getValue().length > 20 || passEmpty){
						this.$.signInButton.setDisabled(true);
					} else {
						this.$.signInButton.setDisabled(false);
					}
	...
	signInPasswordOnKeyPress: function(inSender, inResponse){
		if (inResponse.keyCode == 13) {
			...
			} else if (this.$.signInPassword.getValue().length < 6 || this.$.signInPassword.getValue().length > 20 || passEmpty) {
				this.$.signInPassword.forceFocus();
				this.$.signInPassword.forceSelect();
			} else {
	...
	createPasswordConfirmOnChange: function(inSender, inResponse){
		...
		if (passEmpty) {
			...
			this.$.passwordConfirmText.setShowing(true);
			this.toggleCreateButton(false);			
		} else if(l < 6 || l > 20 || (pl > 0 && (pl < 6 || pl > 20))) {
			this.isErrorDisplayed = true;
	...
	validateInputFields: function(){
		...
		if(!this.verifyPasswords(this.$.createPassword.getValue(), this.$.createPasswordConfirm.getValue()))
			return false;
		if(this.trim(this.$.createAnswer.getValue()).length <1 || this.trim(this.$.createAnswer.getValue()).length > 50)
			return false;
	...
	signIn: function() {
		...
		if (!passEmpty && lenPW >= 6 && lenPW <=20 && this.verifyEmail(this.$.signInEmail.getValue()) && this.verifyEmailLength(this.$.signInEmail.getValue())) {
 ...
 */	
	...
	verifyPasswords: function(pwd1, pwd2){
		if(pwd1.length < 6 || pwd1.length > 20 || pwd2.length < 6 || pwd2.length >20 || pwd1 != pwd2)
			return false;
		return true;
	},
	...
	doPostSignInChecks: function(){
		if (this.createdPostSignIn) {
			this.$.postSignIn.reset();
			console.log("doPostSignInChecks called twice, this should NEVER happen but we have seen it happen once when having network errors. JIRA DFISH-7207.");
		} else {
			this.createdPostSignIn = true;
			this.createComponent({name: "postSignIn", kind:"PostSignIn", onFinish: "postSignInDone"});
		}
	},
